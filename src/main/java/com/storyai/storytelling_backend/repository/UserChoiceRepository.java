...
public interface UserChoiceRepository extends JpaRepository<UserChoice, Long> {
  ...
  /**
   * Find choices by session ordered by creation time
   * UPDATED: timestamp → createdAt
   */
  List<UserChoice> findBySessionOrderByCreatedAt(StorySession session);

  /**
   * Find choices by session ID ordered by creation time
   * UPDATED: timestamp → createdAt
   */
  List<UserChoice> findBySessionIdOrderByCreatedAt(Long sessionId);

  /**
   * Find choices by session and chapter ordered by option number
   * UPDATED: choiceOrder → optionNumber
   */
  List<UserChoice> findBySessionAndChapterOrderByOptionNumber(
    StorySession session, StoryChapter chapter);

  /**
   * Find all choices for a chapter ordered by option number
   * UPDATED: choiceOrder → optionNumber
   */
  ...
  /**
   * Find all choices for a specific chapter, ordered by option number
   * Used to display available choices to user
   */
  List<UserChoice> findByChapterOrderByOptionNumberAsc(StoryChapter chapter);

  /**
   * Find all choices that have been made (chosen) for a specific story
   * Used for choice history
   */
  ...
  /**
   * Find choices by chapter and next chapter number
   * Useful for determining where a choice leads
   */
  List<UserChoice> findByChapterAndNextChapterNumber(StoryChapter chapter, Integer nextChapterNumber);
   ...
   */
  @Query("SELECT c FROM UserChoice c WHERE c.chapter.story = :story AND c.chosenAt IS NOT NULL ORDER BY c.chosenAt DESC")
  List<UserChoice> findChosenChoicesByStory(Story story);

  /**
   * Find all available (unchosen) choices for a specific chapter
   */
  @Query("SELECT c FROM UserChoice c WHERE c.chapter = :chapter AND c.chosenAt IS NULL ORDER BY c.optionNumber ASC")
  List<UserChoice> findAvailableChoicesByChapter(StoryChapter chapter);

  /**
   * Get choice history for a session with full details
   */
  @Query("SELECT c FROM UserChoice c WHERE c.session = :session AND c.chosenAt IS NOT NULL ORDER BY c.chosenAt ASC")
